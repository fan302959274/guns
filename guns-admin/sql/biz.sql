...
CREATE TABLE `wb_attachment` (
	...
	`attachment_type` VARCHAR (40) NOT NULL COMMENT '附件类型1:队员附件;2:球队附件',
	`attachment_key` VARCHAR (40) NOT NULL COMMENT '关联主键:队员id|球队id',
...
CREATE TABLE `wb_park` (
	...
	`park_park_id` BIGINT (11) NOT NULL COMMENT '球场id',
	`park_status` VARCHAR (10) DEFAULT NULL COMMENT '比赛状态(待定)',
	`create_date` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
	`update_date` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
	`creator` VARCHAR (100) DEFAULT NULL COMMENT '创建人',
	`updator` VARCHAR (100) DEFAULT NULL COMMENT '更新人',
